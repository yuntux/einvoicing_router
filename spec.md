...
- À la réception (Odoo → SuperPDP), le routeur agit en proxy transparent (cf. § 4.4) : il ne réinterprète pas les messages de cycle de vie qu'Odoo émet lui-même, mais il doit néanmoins pouvoir **afficher/consulter** dans son IHM les messages de cycle de vie liés aux factures qu'il héberge, qu'ils aient été émis manuellement depuis son IHM propre, transmis par Odoo via le proxy d'émission, ou reçus de SuperPDP.
  - **Ce proxy ne persiste rien en base au moment de l'appel** : le CDAR transmis par Odoo est relayé à SuperPDP et sa réponse est renvoyée telle quelle à Odoo, sans création de `LifecycleEvent`/`AfnorFlow` dans ce même échange (seule la requête/réponse brute est tracée via `FlowTrace`, § 6.1, NF1 — sans le contenu binaire). Le message n'est donc consultable dans l'IHM du routeur qu'après avoir été **re-remonté au cycle de polling suivant** (§ 4.1), au même titre qu'un message de cycle de vie reçu directement de SuperPDP.
  - **Pas de risque de doublon pour un statut saisi manuellement dans l'IHM du routeur** (§ 4.2, envoi immédiat à SuperPDP) lorsqu'il redescend ensuite via le polling : l'identifiant de flux attribué par SuperPDP lors de l'envoi est conservé sur l'`AfnorFlow` créé à la saisie, et l'ingestion du polling déduplique tout flux entrant sur ce même identifiant (indépendamment du sens `in`/`out`, SuperPDP pouvant réexposer un statut sous l'un ou l'autre) avant toute création — le flux redescendu est donc reconnu comme déjà connu et n'engendre pas de second `LifecycleEvent`/`AfnorFlow`.
...
Trois rôles : **admin** (voit toutes les entreprises), **user** (« utilisateur restreint », restreint à son périmètre d'entreprises, cf. § NF4) et **readonly** (« lecture seule »). Le rôle `readonly` a exactement le même périmètre et le même accès aux pages que `user` — mêmes pages visibles, même filtrage par entreprise — mais ne peut **effectuer aucune action d'écriture** : là où le tableau ci-dessous distingue `user`/`readonly`, seule la capacité d'écrire change, jamais ce qui est visible. Le contrôle d'accès est appliqué côté backend (au niveau des pages via `require_admin`/périmètre, et au niveau de chaque écriture via `require_write` pour `readonly`) ; les pages réservées aux admins sont en plus **masquées du menu latéral** pour un utilisateur restreint ou lecture seule (pas seulement bloquées à l'appel), et les actions d'écriture sont masquées/désactivées dans l'IHM pour un utilisateur lecture seule (pas seulement refusées à l'appel).

| Page | Admin | Utilisateur restreint | Lecture seule |
|---|---|---|---|
| **Factures** | Voit toutes les factures de toutes les entreprises ; consultation, téléchargement, saisie d'un événement de cycle de vie sur n'importe quelle facture. | Liste et détail filtrés à son périmètre d'entreprises ; mêmes actions que l'admin, mais uniquement sur les factures de son périmètre. | Même liste/détail filtrés que l'utilisateur restreint (consultation, téléchargement, historique de cycle de vie) ; **ne peut pas saisir d'événement de cycle de vie**. |
| **Entreprises** | Voit toutes les entreprises ; seul rôle pouvant **créer** une entreprise ; consultation/modification des identifiants SuperPDP de n'importe quelle entreprise. | **Page et API réservées aux admins** — aucun accès (403), page masquée du menu. Seule exception : une référence minimale non sensible (id + nom, sans SIREN), accessible en API à tout utilisateur authentifié, alimente l'affichage croisé d'autres pages (ex. Règles de routage) sans donner accès à la page Entreprises elle-même. | Identique à l'utilisateur restreint (page admin-only, même exception de référence minimale). |
| **Applications cibles** | Voit toutes les applications cibles ; peut créer, modifier, activer/désactiver n'importe quelle application. | **Page et API réservées aux admins** — aucun accès (403), page masquée du menu, y compris pour les applications de son propre périmètre. Même exception que ci-dessus : une référence minimale (id + nom + entreprise, sans paramètres ni infos OAuth) reste accessible en API pour l'affichage de la page Règles de routage. | Identique à l'utilisateur restreint (page admin-only, même exception de référence minimale). |
| **Règles de routage** | Voit la matrice complète ; peut cocher/décocher n'importe quelle règle, avec rejeu des factures en attente ; peut ajouter un fournisseur. | Voit la **même matrice complète, non filtrée par périmètre**, et peut cocher/décocher n'importe quelle règle — y compris pour des entreprises hors de son périmètre ; peut ajouter un fournisseur. | Voit la même matrice, non filtrée par périmètre ; **ne peut ni cocher/décocher une règle, ni ajouter un fournisseur** (cases à cocher désactivées, formulaire d'ajout masqué). |
| **Échecs de routage** | Voit tous les échecs/tentatives ; peut rejouer n'importe quel routage ; peut forcer un cycle d'envoi global. | Liste **filtrée à son périmètre** (ne voit que les échecs des entreprises pour lesquelles il est habilité) ; le rejeu d'un routage est vérifié par périmètre (refusé si hors périmètre) ; **peut également forcer un cycle d'envoi global**, affectant toutes les entreprises. | Même liste filtrée que l'utilisateur restreint ; **ne peut ni rejouer un routage, ni forcer un cycle d'envoi global** (actions masquées). |
| **Configuration** | Consultation des réglages et des contacts gestionnaires de facturation ; seul rôle pouvant les **modifier/créer/supprimer**. | **Page et API réservées aux admins** — aucun accès, y compris en lecture (403), page masquée du menu. | Identique à l'utilisateur restreint (page admin-only). |
| **Gestion des accès** | Seul rôle ayant accès à la page : liste des utilisateurs, création de compte par email, modification du rôle/périmètre/statut actif de n'importe quel utilisateur. | **Aucun accès** (403 sur tous les appels), page masquée du menu. | Identique à l'utilisateur restreint (page admin-only). |
| **Traces techniques** | Voit toutes les requêtes/réponses HTTP brutes des appels AFNOR. | **Page et API réservées aux admins** — aucun accès (403), page masquée du menu (le groupe *Traces & journaux* entier disparaît du menu). | Identique à l'utilisateur restreint (page admin-only). |
| **Journal des traitements** | Voit tous les journaux techniques (cycles de polling, etc.) de toutes les entreprises. | **Page et API réservées aux admins** — aucun accès (403). | Identique à l'utilisateur restreint (page admin-only). |
| **Journal d'audit** | Voit toutes les actions utilisateur (connexions, téléchargements, créations/modifications) de tous les utilisateurs, toutes entreprises. | **Page et API réservées aux admins** — aucun accès (403). | Identique à l'utilisateur restreint (page admin-only). |

> **Écart à trancher avec le métier avant de le considérer comme définitif** : la page *Règles de routage* n'est pas filtrée par périmètre pour un utilisateur restreint ou lecture seule — il y voit (et, pour l'utilisateur restreint, agit sur) des règles d'entreprises hors de son périmètre. De même, l'action *forcer un cycle d'envoi global* (page *Échecs de routage*, ouverte à l'utilisateur restreint) n'est soumise à aucune restriction de périmètre au-delà du rôle. Choix assumés (référentiel global, action de maintenance) ou oublis à corriger — à confirmer.
