"""client_factory — sélection fake/pyfrctc selon settings.certified_platform_client_mode
(spec.md § 4.1/§ 4.8, lot 6)."""

from unittest.mock import patch

from app.afnor.client.fake import FakeCertifiedPlatformClient
from app.afnor.client.pyfrctc_client import PyfrctcCertifiedPlatformClient
from app.config import settings
from app.models.referential import Company
from app.services.client_factory import resolve_client_for_company


def test_resolve_client_fake_mode_returns_fake_client(db_session, monkeypatch):
    monkeypatch.setattr(settings, "certified_platform_client_mode", "fake")
    company = Company(siren="123456789", name="Test")
    db_session.add(company)
    db_session.commit()

    client = resolve_client_for_company(db_session, company)
    assert isinstance(client, FakeCertifiedPlatformClient)


def test_resolve_client_pyfrctc_mode_returns_pyfrctc_client(db_session, monkeypatch):
    monkeypatch.setattr(settings, "certified_platform_client_mode", "pyfrctc")
    company = Company(siren="123456789", name="Test")
    db_session.add(company)
    db_session.commit()

    with patch("app.afnor.client.adapter.core.get_session", return_value="fake-session"):
        from app.services import certified_platform_credentials_service

        certified_platform_credentials_service.set_credentials(
            db_session, company_id=company.id, client_id="cid", client_secret="csecret"
        )
        client = resolve_client_for_company(db_session, company)

    assert isinstance(client, PyfrctcCertifiedPlatformClient)


def test_resolve_client_pyfrctc_mode_uses_company_platform_when_set(db_session, monkeypatch):
    """Une entreprise avec un `platform` explicite (§ 4.10) prime sur
    `settings.certified_platform` lors de l'ouverture de session pyfrctc."""
    monkeypatch.setattr(settings, "certified_platform_client_mode", "pyfrctc")
    company = Company(siren="123456789", name="Test")
    db_session.add(company)
    db_session.commit()

    with patch("app.afnor.client.adapter.core.get_session", return_value="fake-session") as get_session:
        from app.services import certified_platform_credentials_service

        certified_platform_credentials_service.set_credentials(
            db_session,
            company_id=company.id,
            client_id="cid",
            client_secret="csecret",
            platform="superpdp",
        )
        resolve_client_for_company(db_session, company)

    assert get_session.call_args.kwargs["platform"] == "superpdp"
