...
from app.services import certified_platform_credentials_service
...
def test_status_unconfigured_by_default(client):
    ...
    response = client.get(f"/api/ihm/companies/{company_id}/certified-platform-credentials")
...
def test_set_credentials_then_status_never_leaks_secret(client, db_session):
    ...
    response = client.put(
        f"/api/ihm/companies/{company_id}/certified-platform-credentials",
    ...
    status = client.get(f"/api/ihm/companies/{company_id}/certified-platform-credentials")
    ...
    assert decrypt_secret(application.certified_platform_client_secret_encrypted) == "s3cret-superpdp"
...
def test_replacing_credentials_clears_token_cache(client, db_session):
    ...
    application.certified_platform_token_cache = '{"access_token": "abc"}'
    db_session.commit()
    ...
    assert application.certified_platform_client_id == "client-b"
    assert application.certified_platform_token_cache is None
...
def test_set_credentials_rejected_when_connection_test_fails(client, db_session, monkeypatch):
    ...
    with patch(
        "app.services.certified_platform_credentials_service.core.get_session",
    ...
    assert certified_platform_credentials_service.get_credentials_application(
        db_session, company_id=company_id
    ) is None
...
def test_set_credentials_accepted_when_connection_test_succeeds(client, monkeypatch):
    ...
    with patch(
        "app.services.certified_platform_credentials_service.core.get_session", return_value="fake-session"
    ):
        response = client.put(
            f"/api/ihm/companies/{company_id}/certified-platform-credentials",
            json={"client_id": "client-a", "client_secret": "secret-a"},
