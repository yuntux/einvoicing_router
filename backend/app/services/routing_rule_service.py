...
def resolve(db: Session, *, siren: str, reference_date: date) -> list[TargetApplication]:
    ...
    for rule in matching:
        if rule.target_application_id not in seen:
            seen.add(rule.target_application_id)
            targets.append(rule.target_application)
    return targets
